...
class Pass extends base_pass_1.PassBase {
    ...
    async asBuffer() {
        ...
        const manifestJson = JSON.stringify(zip.reduce((res, { path, data }) => {
            res[path] = getBufferHash_1.getBufferHash(data);
            return res;
        }, {}));
        zip.push({ path: 'manifest.json', data: manifestJson });
        // Create signature
        const signature = signManifest_forge_1.signManifest(this.template.certificate, this.template.key, manifestJson);
        zip.push({ path: 'signature', data: signature });
        // finished!
        return do_not_zip_1.toBuffer(zip);
    }
}
exports.Pass = Pass;
