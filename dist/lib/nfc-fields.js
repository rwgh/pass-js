...
class NFCField {
     ...
     */
    setPublicKey(key) {
        const pemKey = typeof key === 'string' ? key : forge.pki.publicKeyToPem(key);
        // test PEM key type
        ...
        if (!publicKey)
            throw new TypeError(`NFC publicKey must be a PEM encoded X.509 SubjectPublicKeyInfo string`);
        ...
        this.encryptionPublicKey = Buffer.from(publicKey.body, 'binary').toString('base64');
        return this;
    }
    toJSON() {
        if (!this.message)
...
exports.NFCField = NFCField;
//# sourceMappingURL=nfc-fields.js.map